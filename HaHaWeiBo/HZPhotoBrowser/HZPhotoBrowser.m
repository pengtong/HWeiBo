...
- (void)addScrollView
{
    ...
    for (int i = 0; i < self.imageCount; i++) {
        ...
        view.longPressBlock = ^(UILongPressGestureRecognizer *recognizer){
            __strong __typeof(weakSelf)strongSelf = weakSelf;
            [strongSelf handleLongPressRecognizer:recognizer];
        };
...
- (void)handleLongPressRecognizer:(UILongPressGestureRecognizer *)recognizer
{
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"保存照片" message:nil preferredStyle:UIAlertControllerStyleActionSheet];
